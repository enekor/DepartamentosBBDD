...
@Data
public class ProgramadorModelo {

    DataBaseController dbc = DataBaseController.getInstance();
    private static ProgramadorModelo pm = null;
    private ProgramadorModelo(){
        try {
            dbc.open();
    ...
    public static ProgramadorModelo getInstance() {
        if(pm==null){
            pm=new ProgramadorModelo();
        }
        return pm;
    }

    private Map<Integer,Programador> programadores = new HashMap<>();


    public void newProgramador(int id,double salario,String nombre,int idDepartamento){
        String query = "insert into programador (ID,salario,nombre,idDepartamento) values(?,?,?,?)";
        try {
            dbc.insert(query,id,salario,nombre,idDepartamento);
            Programador programador = new Programador().builder().id(id).nombre(nombre).salario(salario).build();
            if(DepartamentoModelo.getInstance().getDepartamentos().containsKey(idDepartamento)){
                programador.setTrabajo(DepartamentoModelo.getInstance().getDepartamentos().get(idDepartamento));
            }else System.err.println("no se ha encontrado ningun departamento con id "+idDepartamento);
            programadores.put(id,programador);

        } catch (SQLException e) {
            System.err.println("no se pudo aniadir el programador solicitado");
        }
    }

    public void newProgramador(Programador programador){
        String query = "insert into POJO.Departamento(ID,nombre,presupuesto,idJefe) values(?,?,?,?)";
        try {
            dbc.insert(query,programador.getId(), programador.getNombre(),programador.getSalario(),programador.getTrabajo().getId());
            programadores.put(programador.getId(),programador);
        } catch (SQLException e) {
            System.err.println("no se pudo aniadir el programador solicitado");
        }
    }
